...
class seguricel_prototipo(MDApp):
    ...
    def cambiar_entradas(self, instance):
        ...
        try:
            for acceso in peatonales:
                ...
                if 'entrada' in peatonales[acceso].lower() and 'salida' in peatonales[acceso].lower():
                    print('cambiar entradas')
                    if 'entrada' in peatonales[acceso].lower():
                        if acceso == 1: 
                            btn.bind(on_press=self.enviar_peticion_acceso1entrar)
                            btn.color = (0, 1, 0, 1)
                            self.layout_entradas.add_widget(btn)
                    if 'salida' in peatonales[acceso].lower():
                        if acceso == 1: 
                            btn0.bind(on_press=self.enviar_peticion_acceso1salir)
                           
                            self.layout_salidas.add_widget(btn0)
                else:
                    if int(acceso) == 1: 
                        btn.bind(on_press=self.enviar_peticion_acceso1)
                    if int(acceso) == 2:       
        ...
        if self.id_usuario.text:
            ...
            try:
                ...
                for acceso in accesosPeatonalesConDescripcion:
                    ...
                    if 'entrada' in accesosPeatonalesConDescripcion[acceso].lower() and 'salida' in accesosPeatonalesConDescripcion[acceso].lower():
                        ...
                        if 'entrada' in accesosPeatonalesConDescripcion[acceso].lower():
                            if acceso == 1: 
                                ...
                                self.layout_entradas.add_widget(btn)
                            if acceso == 2: 
                                btn.bind(on_press=self.enviar_peticion_acceso2entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 3: 
                                btn.bind(on_press=self.enviar_peticion_acceso3entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 4: 
                                btn.bind(on_press=self.enviar_peticion_acceso4entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 5: 
                                btn.bind(on_press=self.enviar_peticion_acceso5entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 6: 
                                btn.bind(on_press=self.enviar_peticion_acceso6entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 7: 
                                btn.bind(on_press=self.enviar_peticion_acceso7entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 8: 
                                btn.bind(on_press=self.enviar_peticion_acceso8entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 9: 
                                btn.bind(on_press=self.enviar_peticion_acceso9entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                            if acceso == 10: 
                                btn.bind(on_press=self.enviar_peticion_acceso10entrar)
                                btn.color = (0, 1, 0, 1)
                                self.layout_entradas.add_widget(btn)
                        if 'salida' in accesosPeatonalesConDescripcion[acceso].lower():
                            if acceso == 1: 
                                btn0.bind(on_press=self.enviar_peticion_acceso1salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 2: 
                                btn0.bind(on_press=self.enviar_peticion_acceso2salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 3: 
                                btn0.bind(on_press=self.enviar_peticion_acceso3salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 4: 
                                btn0.bind(on_press=self.enviar_peticion_acceso4salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 5: 
                                btn0.bind(on_press=self.enviar_peticion_acceso5salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 6: 
                                btn0.bind(on_press=self.enviar_peticion_acceso6salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 7: 
                                btn0.bind(on_press=self.enviar_peticion_acceso7salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 8: 
                                btn0.bind(on_press=self.enviar_peticion_acceso8salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 9: 
                                btn0.bind(on_press=self.enviar_peticion_acceso9salir) 
                                self.layout_salidas.add_widget(btn0)
                            if acceso == 10: 
                                btn0.bind(on_press=self.enviar_peticion_acceso10salir) 
                                self.layout_salidas.add_widget(btn0)
                    else:
                        if acceso == 1: 
                            btn.bind(on_press=self.enviar_peticion_acceso1)
    ...
    def enviar_peticion_acceso1entrar(self, obj):
        contrato=''
        usuario_id=''
        try:
            contrato = store.get('datos_usuario')['contrato']
            ...
            if contrato and usuario_id:
                if not modoInternet:
                    try:
                        ...
                        print(f"{self.servidorLocal}:43157/{usuario_id}/1/seguricel_wifi_activo/entrar")
    ...
    def enviar_peticion_acceso1salir(self, obj):
        contrato=''
        usuario_id=''
        try:
            contrato = store.get('datos_usuario')['contrato']
            ...
            if contrato and usuario_id:
                if not modoInternet:
                    try:
                        #requests.get(url=f"{URL}seguricel_wifi_activo", timeout=3)
                        print(f"{self.servidorLocal}:43157/{usuario_id}/1/seguricel_wifi_activo/salir")
                        requests.post(url=f"{self.servidorLocal}:43157/{usuario_id}/1/seguricel_wifi_activo", timeout=3)
                    except:
                        self.dialogo.text='No fue posible enviar la peticion'
                        self.dialogo.open()
